...
# Calculate project directory for dataset paths
_project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_default_input_path = os.path.join(_project_dir, "datasets", "dataset.hdf5")
_default_output_path = os.path.join(_project_dir, "datasets", "dataset_annotated.hdf5")

# add argparse arguments
parser = argparse.ArgumentParser(description="Annotate demonstrations for Isaac Lab environments.")
parser.add_argument("--task", type=str, default=None, help="Name of the task.")
parser.add_argument(
    "--input_file", type=str, default=_default_input_path, help="File name of the dataset to be annotated."
)
parser.add_argument(
    "--output_file",
    type=str,
...
# Import custom tasks so their envs register (requires PYTHONPATH set accordingly).
...
def replay_episode(
    ...
    initial_state = episode.data.get("initial_state", None)
    ...
    if initial_state is not None:
        ...
        # Fallback when initial_state is missing in the dataset.
        env.reset()
    first_action = True
